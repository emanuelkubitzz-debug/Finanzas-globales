...
// Configurar entorno de PayPal (Sandbox por defecto para desarrollo)
...
// -----------------------------------------------------------------------------
// 6. RUTAS DE PAYPAL (PROCESAMIENTO DE PAGOS)
// -----------------------------------------------------------------------------

// Crear orden de pago en PayPal
app.post('/api/checkout/create-order', authenticateToken, async (req, res) => {
  if (!paypalClient) return res.status(500).json({ error: 'PayPal no está configurado en el servidor.' });
    ...
    purchase_units: [
      {
        ...
        custom_id: String(req.user.id) // Vinculamos el ID del usuario aquí
      }
    ],
    application_context: {
  ...
  try {
    ...
    // Buscar el link de aprobación para redirigir al usuario desde el frontend
    ...
    res.status(500).json({ error: 'Error al generar la pasarela de pago con PayPal.' });
  }
});

// Capturar el pago aprobado por el usuario (Generalmente se invoca al volver de PayPal)
  ...
  try {
    ...
    if (status === 'COMPLETED') {
      ...
      db.get(`SELECT id FROM payments WHERE payment_id = ?`, [captureId], (err, row) => {
        if (row) return res.json({ mensaje: 'El pago ya fue acreditado anteriormente.', status });

        // Registrar pago y actualizar saldo
        db.run(
          `INSERT INTO payments (payment_id, user_id, amount, status) VALUES (?, ?, ?, ?)`,
          [captureId, userId, amount, status],
          (err) => {
